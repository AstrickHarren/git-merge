...
export default class GitMergeView {
  ...
  constructor() {
    ...
    this.btn.addEventListener('click', () => Git.merge(this.select.value, atom.project.getRepositories()[0].getWorkingDirectory()));
  ...
  updateSelection(select) {
    const branches = Git.getBranch(atom.project.getRepositories()[0].getWorkingDirectory());
    select.innerHTML = '';
